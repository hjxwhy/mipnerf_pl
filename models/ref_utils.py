...
import numpy as np
import torch
pi = torch.as_tensor(np.pi, device='cuda:0')

def tensorize(a):
  return torch.as_tensor(a, device='cuda:0')

def factorial(a):
  if type(a) == int:
    a = tensorize(a)
  return torch.lgamma(a+1).exp()
...
def compute_weighted_mae(weights, normals, normals_gt):
  ...
  return (weights * torch.arccos(
      torch.clip((normals * normals_gt).sum(-1), -one_eps,
               one_eps))).sum() / weights.sum() * 180.0 / pi


def generalized_binomial_coeff(a, k):
  """Compute generalized binomial coefficients."""
  return torch.prod(a - torch.arange(k, device=a.device)) / factorial(k)
...
def assoc_legendre_coeff(l, m, k):
  ...
  """
  return ((-1)**m * 2**l * factorial(l) / factorial(k) /
          factorial(l - k - m) *
...
def sph_harm_coeff(l, m, k):
  ...
  return (torch.sqrt(
      (2.0 * l + 1.0) * factorial(l - m) /
      (4.0 * pi * factorial(l + m))) * assoc_legendre_coeff(l, m, k))
...
def get_ml_array(deg_view):
  ...
  ml_array = torch.from_numpy(np.array(ml_list).T).to('cuda:0') ## FIX THIS
  return ml_array
...
def generate_ide_fn(deg_view):
  ...
  mat = torch.zeros((l_max + 1, ml_array.shape[1]), device='cuda:0')
  ...
  def integrated_dir_enc_fn(xyz, kappa_inv):
    ...
    vmz = torch.concatenate([z**i for i in range(mat.shape[0])], axis=-1) ###CORRECT THSI

    # Compute x+iy Vandermonde matrix.
    vmxy = torch.concatenate([(x + 1j * y)**m for m in ml_array[0, :]], axis=-1)
