...
def lift_gaussian(directions, t_mean, t_var, r_var, diagonal):
    ...
    if diagonal:
        ...
        d_outer = directions[..., :, None] * directions[..., None, :]  # [B, 3, 1] * [B, 1, 3] = [B, 3, 3]
        ...
        t_cov = t_var[..., None, None] * d_outer[..., None, :, :]  # [B, N, 1, 1] * [B, 1, 3, 3] = [B, N, 3, 3]
        xy_cov = r_var[..., None, None] * null_outer[..., None, :, :]  # [B, N, 1, 1] * [B, 1, 3, 3] = [B, N, 3, 3]
        cov = t_cov + xy_cov
        return mean, cov
...
def volumetric_rendering(rgb, density, t_samples, dirs, white_bkgd):
    ...
    comp_rgb = (weights[..., None] * rgb).sum(axis=-2)  # [B, N, 1] * [B, N, 3] -> [B, 3]
